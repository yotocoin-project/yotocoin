...
uint256 hashGenesisBlock("0x7b9ccd0a03035a4fac5c5436c57909ef2443bd67b94ff711bd38e7eb8be1ce7f");
...
const string strMessageMagic = "yotocoin Signed Message:\n";
...
{
    // yotocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // yotocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "yoto genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xda, 0xc4, 0xa5, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("yotocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("yotocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static yotocoinMiner(CWallet *pwallet)
{
    printf("yotocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("yotocoin-miner");
    ...
    try { loop {
        ...
        printf("Running yotocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("yotocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&yotocoinMiner, pwallet));
}
