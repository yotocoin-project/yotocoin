...
// Copyright (c) 2011-2012 yotocoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #yotocoinTEST0\r");
            Send(hSocket, "WHO #yotocoinTEST0\r");
        } else {
            // randomly join #yotocoin00-#yotocoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #yotocoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #yotocoin%02d\r", channel_number).c_str());
        }
